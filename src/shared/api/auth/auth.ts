/**
 * Generated by orval v7.1.1 🍺
 * Do not edit manually.
 * Anime ON API
 * OpenAPI spec version: ##VERSION## DEV
 */
import type {
  CustomTokenObtainPair,
  RequestUserRegister,
  ResponseUserLogin,
  ResponseUserLogout,
  ResponseUserRegister,
  TokenBlacklist,
  TokenRefresh
} from '.././model'
import { customInstance } from '.././custom-instance';

// https://stackoverflow.com/questions/49579094/typescript-conditional-types-filter-out-readonly-properties-pick-only-requir/49579497#49579497
type IfEquals<X, Y, A = X, B = never> = (<T>() => T extends X ? 1 : 2) extends <
T,
>() => T extends Y ? 1 : 2
? A
: B;

type WritableKeys<T> = {
[P in keyof T]-?: IfEquals<
  { [Q in P]: T[P] },
  { -readonly [Q in P]: T[P] },
  P
>;
}[keyof T];

type UnionToIntersection<U> =
  (U extends any ? (k: U)=>void : never) extends ((k: infer I)=>void) ? I : never;
type DistributeReadOnlyOverUnions<T> = T extends any ? NonReadonly<T> : never;

type Writable<T> = Pick<T, WritableKeys<T>>;
type NonReadonly<T> = [T] extends [UnionToIntersection<T>] ? {
  [P in keyof Writable<T>]: T[P] extends object
    ? NonReadonly<NonNullable<T[P]>>
    : T[P];
} : DistributeReadOnlyOverUnions<T>;


/**
 * Ellipsis
 * @summary Ellipsis
 */
export type userLoginResponse = {
  data: ResponseUserLogin;
  status: number;
}

export const getUserLoginUrl = () => {


  return `/auth/login/`
}

export const userLogin = async (customTokenObtainPair: CustomTokenObtainPair, options?: RequestInit): Promise<userLoginResponse> => {
  
  return customInstance<Promise<userLoginResponse>>(getUserLoginUrl(),
  {      
    ...options,
    method: 'POST',
    headers: { 'Content-Type': 'application/json', ...options?.headers },
    body: JSON.stringify(
      customTokenObtainPair,)
  }
);}


/**
 * Ellipsis
 * @summary Ellipsis
 */
export type userLogoutResponse = {
  data: ResponseUserLogout;
  status: number;
}

export const getUserLogoutUrl = () => {


  return `/auth/logout/`
}

export const userLogout = async (tokenBlacklist: TokenBlacklist, options?: RequestInit): Promise<userLogoutResponse> => {
  
  return customInstance<Promise<userLogoutResponse>>(getUserLogoutUrl(),
  {      
    ...options,
    method: 'POST',
    headers: { 'Content-Type': 'application/json', ...options?.headers },
    body: JSON.stringify(
      tokenBlacklist,)
  }
);}


/**
 * Ellipsis
 * @summary Ellipsis
 */
export type userRegisterResponse = {
  data: ResponseUserRegister;
  status: number;
}

export const getUserRegisterUrl = () => {


  return `/auth/registration/`
}

export const userRegister = async (requestUserRegister: RequestUserRegister, options?: RequestInit): Promise<userRegisterResponse> => {
  
  return customInstance<Promise<userRegisterResponse>>(getUserRegisterUrl(),
  {      
    ...options,
    method: 'POST',
    headers: { 'Content-Type': 'application/json', ...options?.headers },
    body: JSON.stringify(
      requestUserRegister,)
  }
);}


/**
 * Takes a refresh type JSON web token and returns an access type JSON web
token if the refresh token is valid.
 */
export type authTokenRefreshCreateResponse = {
  data: TokenRefresh;
  status: number;
}

export const getAuthTokenRefreshCreateUrl = () => {


  return `/auth/token/refresh/`
}

export const authTokenRefreshCreate = async (tokenRefresh: NonReadonly<TokenRefresh>, options?: RequestInit): Promise<authTokenRefreshCreateResponse> => {
  return customInstance<Promise<authTokenRefreshCreateResponse>>(getAuthTokenRefreshCreateUrl(),
  {      
    ...options,
    method: 'POST',
    headers: { 'Content-Type': 'application/json', ...options?.headers },
    body: JSON.stringify(
      tokenRefresh,)
  }
);}


